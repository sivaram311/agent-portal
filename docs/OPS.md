...
`PushNotificationService.notifyOwner(...)` is called from `SessionEventBus`, resolving the session owner via `AgentSessionRepository`. Forwarded events: the same four `WebhookService` fires (`run_completed`/`run_failed`/`input_required`/`run_cancelled`) **plus** `permission_required`/`plan_required` — the actual Cursor ACP tool/plan-approval events that carry a real `permissionId` (`input_required` is Antigravity's free-text nudge and never has one; a prior version of this wiring only forwarded the original four, so a real permission request never triggered a push — fixed 2026-07-27). The webhook contract itself (`WebhookService.publish`) is unchanged — only push gets the extra two event types.

**Firebase is live.** `app.firebase.credentials-path` (env `FIREBASE_CREDENTIALS_PATH`) points at a service-account JSON; when set and the file exists, `PushNotificationService` initializes the Firebase Admin SDK on boot and really sends via `FirebaseMessaging`. Empty/missing → stays log-only, no crash (unchanged fallback behavior). DEV's credential lives at `E:\MyAgent\workflow\secrets\firebase-admin-agent-portal.json` (machine-local, gitignored, never committed) and is picked up automatically by `scripts/start-dev-backend.ps1` if present. Stale/unregistered tokens (FCM `UNREGISTERED` error) are auto-deleted from `device_tokens`. Table is Hibernate `ddl-auto=update`-managed (no Flyway migration needed).
